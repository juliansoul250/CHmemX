...
默认向量器使用 NFKC 规范化、英文单词、中文 2/3 字符片段和 SHA-256 特征哈希。每次重建会从全部 accepted + Active 记忆计算 IDF，并分别生成内容网格、路由节点和记录级向量。召回综合记录相似度、节点相似度、网格路由、精确短语、当前项目上下文和一跳关系。它仍是词法稀疏向量，不是神经网络语义嵌入。

正式索引发布前执行独立质量门禁：自动生成的覆盖集检查每条 Active 记录和节点；策展者维护的脱敏黄金查询检查 Top-1、Hit@3、MRR、Precision@K、禁止命中和项目范围违规。优化器只比较受限且确定性的评分参数，全部达标才覆盖正式索引。
